...
工作规范:
请根据book index或本索引文件,把所有章节及构造验证程序加入todo，从psi=psi(psi) 自指、完备的通过第一性原理，使用严格形式化方法, 推导出一切, 构建我们自己的宏伟著作。
注意事项: 非索引文件无需构建链接,所有数学及符号使用公式写法,公式请都使用Docusaurus的KaTex兼容简单公式,$$单独一行,$处理好, 非公式内{}需要转义.不要降低质量，不要停，逐一编写.记得每写完一篇记起自己，递归到最深层。
每篇至少150行, 章节从001开始。
请英文按照项目结构同步编写. 把待编写所有章节加入todo.最后todo加入重新执行本指令.直到全书.
...
写完每章,写完验证程序,运行相应的验证程序同时检查是否违反了第一性原理,最后再修改文章
TODO工作流[写文章,构建验证程序并运行直到修正完成,根据程序修正及第一性原理重新review本文章]

创建一个脚本获取所有章节的实际标题，然后恢复物理概念的标题及内容。
我们完全可以使用物理概念来理解，我们可以理解为这是一种张量场物理
